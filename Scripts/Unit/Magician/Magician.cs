...
public class Magician : Unit, ISetTarget
{
    enum State
    {
    ...
    private void Awake()
    {
        ...
        this.NumberOfProjectile = Data.NumberOfProjectile;
    ...
    private IEnumerator AttackCoroutine()
    {
        ...
        while (true)
        {
            for (int i = 0; i < NumberOfProjectile; i++)
            {
                ...
                GameObject projectile = Instantiate(Data.ProjectilePrefab, transform.position, rotation);
                EnergyBall energyBall = projectile.GetComponent<EnergyBall>();
                energyBall.SetProjectileSpeed(this.ProjectileSpeed);
                energyBall.SetDamage(this.Damage);
                energyBall.SetDirection(GetDirctionToTarget());
                energyBall.SetTargetLayer(this.TargetLayer);
               
                yield return WaitFor02S;
            }
