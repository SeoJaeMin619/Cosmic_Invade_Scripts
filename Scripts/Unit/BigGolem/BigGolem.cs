...
public class BigGolem : Unit, ISetTarget
{
    enum State
    {
    ...
    //Ray 
    [SerializeField] private Transform RayResultPoint;
    [SerializeField] private Transform RayStartPoint;
    [SerializeField] private Transform RaySprite;
    [SerializeField] private SpriteRenderer RaySpriteRenderer;
    //Attack Target
    ...
    private IEnumerator AttackCoroutine()
    {
        ...
        while (true)
        {
            ...
            Vector2 RayDir = (Target.position - RayStartPoint.position).normalized;
            hit = Physics2D.Raycast(RayStartPoint.position, RayDir, 50f, TargetLayer);
            RaySprite.transform.localScale = new Vector3(hit.distance +1f, 2, 1);
            RayResultPoint.transform.position = hit.point;
            Vector3 dirToHitPoint = new Vector3(hit.point.x - RayStartPoint.position.x, hit.point.y - RayStartPoint.position.y ,0f);
            float angle = Mathf.Atan2(dirToHitPoint.y, dirToHitPoint.x) * Mathf.Rad2Deg;
            angle += 180;
            RayStartPoint.rotation = Quaternion.Euler(0, 0, angle);
    ...
    private void ExitState(State CuState)
    {
        if (CuState == State.Attack)
        {
            ...
            RaySpriteRenderer.enabled = false;
    ...
    private void EnterState(State InputState)
    {
        if (cuState == State.Attack)
        {
            ...
            DetectCollider.enabled = false;
            RaySpriteRenderer.enabled = true;
